...
from model.utils.bbox_ops import loc2bbox
...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1.):
        ...
        features1 = self.extractor1(x)
        rpn_coords, rpn_scores, rois, roi_indices, anchor = self.rpn(features1, img_size, scale)
        
        features2 = self.extractor2(x)
        roi_cls_locs, roi_scores = self.head(features2, rois, roi_indices)
    ...
    def predict(self, imgs,sizes=None,visualize=False):
        ...
        with torch.no_grad():
            ...
            for img, size in zip(prepared_imgs, sizes):
                ...
                cls_bbox = loc2bbox(roi.detach().cpu().numpy().reshape((-1, 4)),

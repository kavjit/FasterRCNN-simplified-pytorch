...
class TrainStep(nn.Module):
    ...
    def forward(self, inp_img, bboxes, labels, scale):
        ...
        features1 = self.fasterrcnn.extractor1(inp_img)
        features2 = self.fasterrcnn.extractor2(inp_img)

        rpn_locs, rpn_scores, rois, roi_indices, anchor = self.fasterrcnn.rpn(features1, img_size, scale)
        ...
        roi_cls_loc, roi_score = self.fasterrcnn.head(
            features2,
            sample_roi,
            sample_roi_index)
        ...
        losses = [rpn_reg_loss, rpn_classifier_loss, roi_loc_loss, roi_cls_loss]
        losses = losses + [sum(losses)] + [losses[0]+losses[1]] + [losses[2]+losses[3]]
        all_losses = Losses(*losses)
        ######################################################
        ####Backprop for rpn and its extractor
        #freezing weight of head and its extractor
        for param in self.faster_rcnn.extractor2.parameters():
            param.requires_grad = False
        for param in self.faster_rcnn.head.parameters():
            param.requires_grad = False
        all_losses.total_rpn.backward()
        ####Backprop for head and its extractor
        #Freezing weight of rpn and its extractor
        for param in self.faster_rcnn.extractor1.parameters():
            param.requires_grad = False
        for param in self.faster_rcnn.rpn.parameters():
            param.requires_grad = False
        #unfreezing weight of head and its extractor
        for param in self.faster_rcnn.extractor2[10:].parameters():
        ...
        ###Unfrezing weight of rpn and its extractor back
        for param in self.faster_rcnn.extractor1[10:].parameters():
            param.requires_grad = True
        for param in self.faster_rcnn.rpn.parameters():
            param.requires_grad = True 
        #######################################################
